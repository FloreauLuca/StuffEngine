...
void MusicNoise::Init()
{
	...
	if (!waveSoundBuffer_.loadFromFile(dataPath + "note3.wav"))
	{
...
void MusicNoise::Update(float dt)
{
	...
	float r = (squareCount_.x / 3.0f);
	pos_.x = r * sin(timer_);
	pos_.y = r * sin(timer_) * cos(timer_);
	pos_.x += squareCount_.x / 2.0f;
	pos_.y += squareCount_.y / 2.0f;
