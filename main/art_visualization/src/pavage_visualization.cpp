...
namespace stuff
{
	...
	void PavageVisualization::Update(float dt)
	{
		...
		DrawPavage(center + sf::Vector2f(0.0f, 200.0f), sf::Color::White);
		...
		for (size_t x = 0; x < squareCount.x; x++)
		{
			for (size_t y = 0; y < squareCount.y; y++)
			{
				...
				sf::Vector2f center = sf::Vector2f(x * 2.0f * size_, y * 2.0f * size_);
				sf::Color color = (x + y) % 2 == 0 ? sf::Color::Cyan : sf::Color::Red;
				DrawPavage(center, color);
			}
		}
		...
		if (true)
		{
			...
			ImGui::DragFloat2("Speed", speed_, 0.1f, -5.0f, 5.0f);
			ImGui::DragFloat2("Amplitude", amplitude_, 0.1f, -5.0f, 5.0f);
			ImGui::DragFloat2("Frequency", frenquency_, 1.0f, -5.0f, 5.0f);
			ImGui::DragFloat("Tiling", &tilling_, 0.1f, -5.0f, 5.0f);
			ImGui::End();
		}
	}

	void PavageVisualization::DrawPavage(sf::Vector2f position, sf::Color color)
	{
		sf::VertexArray base_vertices(sf::TriangleFan);
		base_vertices.append(sf::Vertex(sf::Vector2f(0.0f, 0.0f) * size_, sf::Color::White));
		AddHorizontalLine(&base_vertices, corners_[0] * size_, corners_[1] * size_, true);
		AddVerticalLine(&base_vertices, base_vertices[base_vertices.getVertexCount()-1].position, corners_[2] * size_, true);
		AddHorizontalLine(&base_vertices, base_vertices[base_vertices.getVertexCount()-1].position, corners_[3] * size_, false);
		AddVerticalLine(&base_vertices, base_vertices[base_vertices.getVertexCount()-1].position, corners_[0] * size_, false);
	...
	void PavageVisualization::AddHorizontalLine(sf::VertexArray* vertices, sf::Vector2f start, sf::Vector2f end, bool reverse)
	{
		...
		for (size_t i = 0; i <= resolution_; i++)
		{
			...
			float curve = sin(frenquency_[0] * lerpValue * 2 * PI + timer) - sin(timer);
			curve *= amplitude_[0];
	...
	void PavageVisualization::AddVerticalLine(sf::VertexArray* vertices, sf::Vector2f start, sf::Vector2f end, bool reverse)
	{
		sf::Vector2f normal = start - end;
		if (reverse)
		{
