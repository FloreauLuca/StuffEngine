...
namespace stuff
{
	...
	{
		timer_ += dt * speed_;
		currentZoom_ = std::exp(lerp(std::log(startZoom_), std::log(endZoom_), cos(timer_ + PI) * 0.5f + 0.5f));
		for (size_t i = 0; i < cameraParameters_.size(); i++)
		{
			if (i < cameraParameters_.size() - 1)
			{
				...
				{
					float value = smoothstep(cameraParameters_[i].zoom, cameraParameters_[i + 1].zoom, currentZoom_);
