...
{
	sf::Vector2f squareSize = sf::Vector2f(windowSize_.x / squareCount_.x, windowSize_.y / squareCount_.y);
	for (unsigned x = 0; x < windowSize_.x; x += squareSize.x)
	{
		for (unsigned y = 0; y < windowSize_.y; y += squareSize.y)
		{
			sf::RectangleShape rect(squareSize);
			sf::Vector2f pos(x, y);
			rect.setPosition(pos);
			auto it = std::ranges::find_if(pixelToDraw_, [pos](std::pair<sf::Vector2f, float> i)
			{
				return i.first == pos;
			});
			const sf::Vector2f length = sf::Vector2f(sf::Mouse::getPosition(*graphics_.GetWindow())) - pos;
			if (std::sqrt(length.x * length.x + length.y * length.y) < radiusMouse * std::sqrt(
				squareSize.x * squareSize.x + squareSize.y * squareSize.y))
			{
				float colorGradient = std::clamp(
					std::lerp(1.0f, 0.0f,
					          std::sqrt(length.x * length.x + length.y * length.y) / (radiusMouse * std::sqrt(
						          squareSize.x * squareSize.x + squareSize.y * squareSize.y))) * mousePower, 0.0f,
					1.0f);
